...
- **Event type**: `Event_Type_Taxonomy` registers the public, flat, REST-visible `event_type` taxonomy and owns its **closed vocabulary**. Terms are seeded from `EventSchemaProvider::default_event_type_vocabulary()` (the Schema.org `EVENT_TYPES`), each carrying its `@type` in `_schema_type` term meta. Consumers replace the list via `data_machine_events_event_type_vocabulary`; site-specific editorial vocabulary belongs in the consumer, never here. Enforcement uses DM core's `datamachine_taxonomy_tool_parameter` (string + vocabulary-derived `enum`) and `datamachine_taxonomy_assign_value` (resolve to an existing term ID or the default, never create), so an invented AI value can never create a term. See `docs/event-type-taxonomy.md`.
...
- `EventSchemaProvider`: Defines `getCoreToolParameters()`, `getSchemaToolParameters()`, `getFieldKeys()`, and `generateSchemaOrg()` so Schema.org JSON-LD merges block attributes, venue meta, and taxonomy data. JSON-LD `@type` is derived from the assigned `event_type` term's `_schema_type` meta; the `eventType` block attribute is derived output and only a fallback for events that predate the taxonomy.
