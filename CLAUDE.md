...
- **VisionExtractor** (@since 0.9.18): Final fallback when structured data and HTML extraction fail. Uses `ImageCandidateFinder` to score potential flyer images based on size, context, and keyword heuristics. `VisionExtractionProcessor` downloads candidates and calls AI vision API to extract event details from promotional flyers. Supports multi-event flyers (e.g., monthly calendars).
